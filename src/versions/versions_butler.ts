...
import {isDeveloper, getAuth} from '../utils/auth';
...
versionsRouter.get('/', async (req, res) => {
    ...
    users.forEach((user: User) => {
        user.devices.forEach((device: Device) => {
            const version = device.appVersion === undefined ? 'undef.' : device.appVersion;
...
const getVersions = (appVersions: any, length: number): string => {
    let htmlString = '';
    Object.keys(appVersions).sort((v1: string, v2: string) => {
        const v1Code = parseInt(v1.split('+')[1]);
        const v2Code = parseInt(v2.split('+')[1]);
